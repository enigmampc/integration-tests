...
describe("Enigma tests", () => {
  ...
  it(
    "should execute compute task: voter 1 casting vote",
    async () => {
      ...
      task1 = await new Promise((resolve, reject) => {
        enigma
          .computeTask(taskFn, taskArgs, taskGasLimit, taskGasPx, accounts[0], votingAddr, constants.RETRIES_COMPUTE)
  ...
  it(
    "should get the confirmed task success",
    async () => {
      do {
        await sleep(1000);
        task1 = await enigma.getTaskRecordStatus(task1);
  ...
  it(
    "should fail to execute compute task: voter 1 casting vote to poll again",
    async () => {
      const pollId = (await votingETHContract.methods.getPolls().call()).length - 1;
      let taskFn = "cast_vote(uint256,bytes32,uint256)";
      let taskArgs = [[pollId, "uint256"], [addr1, "bytes32"], [0, "uint256"]];
      let taskGasLimit = 1000000;
      let taskGasPx = utils.toGrains(1);
  ...
  it(
    "should execute compute task: voter 2 casting vote",
    async () => {
      const pollId = (await votingETHContract.methods.getPolls().call()).length - 1;
      let taskFn = "cast_vote(uint256,bytes32,uint256)";
      let taskArgs = [[pollId, "uint256"], [addr2, "bytes32"], [0, "uint256"]];
      let taskGasLimit = 1000000;
      let taskGasPx = utils.toGrains(1);
      task2 = await new Promise((resolve, reject) => {
        enigma
  ...
  let task3;
  it(
    "should fail to execute compute task to tally poll when poll has not expired",
    async () => {
      const pollId = (await votingETHContract.methods.getPolls().call()).length - 1;
      let taskFn = "tally_poll(uint256)";
      let taskArgs = [[pollId, "uint256"]];
      let taskGasLimit = 1000000;
      let taskGasPx = utils.toGrains(1);
  ...
  let task4;
  it(
    "should execute compute task: tally poll",
    async () => {
      const polls = await votingETHContract.methods.getPolls().call();
      const pollId = polls.length - 1;
      expect(polls[pollId].status).toEqual("1");
