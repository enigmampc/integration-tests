...
describe("Enigma tests", () => {
  ...
  const millionaireAddr = fs.readFileSync("/tmp/enigma/addr-millionaire.txt", "utf-8");
  ...
  it(
    ...
    async () => {
      ...
      let taskArgs = [[addr1, "bytes32"], [1000000, "uint256"]];
  ...
  it(
    ...
    async () => {
      let taskFn = "add_millionaire(bytes32,uint256)";
      let taskArgs = [[addr2, "bytes32"], [2000000, "uint256"]];
      let taskGasLimit = 1000000;
      let taskGasPx = utils.toGrains(1);
  ...
  it(
    "should get and validate the result",
    async () => {
      task3 = await new Promise((resolve, reject) => {
        enigma
          .getTaskResult(task3)
