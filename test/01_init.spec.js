...
describe("Init tests", () => {
  ...
  it("initializes Sample contract", async () => {
    sampleContract = new enigma.web3.eth.Contract(SampleContract["abi"], SampleContract.networks["4447"].address);
    expect(sampleContract.options.address).toBeTruthy;

    fs.writeFile(path.join(homedir, ".enigma", "addr-sample.txt"), sampleContract.options.address, "utf8", function(
      err
    ) {
      if (err) {
  ...
  let workerAddress = [];
  it(
    "should check that " + nodes + " worker(s) and the principal node, and only them, are registered",
    async () => {
      let workerStatuses = [];
      for (let i = 0; i < 10; i++) {
  ...
  it("should deposit tokens in worker banks", async () => {
    ...
    for (let i = 0; i < nodes; i++) {
      let promise = new Promise((resolve, reject) => {
        enigma.admin
          .deposit(accounts[i], utils.toGrains(deposit))
  ...
  it(
    "should move forward epochSize blocks by calling dummy contract",
    async () => {
  ...
  const userPubKey =
    "2ea8e4cefb78efd0725ed12b23b05079a0a433cc8a656f212accf58672fee44a20cfcaa50466237273e762e49ec" +
  ...
  it("should create getTaskEncryptionKey from core (with call to P2P)", async () => {
    const encryptionKeyResult = await new Promise((resolve, reject) => {
      enigma.client.request(
        ...
        { workerAddress: workerAddress[0].toLowerCase().slice(-40), userPubKey: userPubKey },
        (err, response) => {
          if (err) {
            reject(err);
          }
